...
class neuralNet{
	...
	train(input,output){
		...
		for(let k=this.hidn.length+1;k>=0;k--){
			...
			err[k]=matrix.dot(this.weights[k].transpose(),err[k+1]);
		};
		let update=0;
		for(let k=0;k<this.hidn.length+1;k++){
			this.weights[k]=matrix.sum(this.weights[k],matrix.scmultiply(this.lr,matrix.dot(matrix.multiply(err[k+1],matrix.substract(out[k+1],matrix.multiply(out[k+1],out[k+1]))),out[k].transpose())));
